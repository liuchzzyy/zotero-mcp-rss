...
| `cli.py` | `main()` | CLI 入口，支持 serve/setup/scan/update-db/db-status/db-inspect/update/update-metadata/deduplicate/pdf-find/pdf-find-batch/version/setup-info 等子命令 |
...
| `pdf-find` | 查找 PDF 与 Supporting Information（单条或批量自动识别） |
| `pdf-find-batch` | 批量查找 PDF 与 Supporting Information |
...
| `services/zotero/metadata_service.py` | DOI/元数据补全（Crossref/OpenAlex，优先级：DOI → 标题 → URL） |
